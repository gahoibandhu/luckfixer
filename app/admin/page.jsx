...
export default function AdminPage() {
  ...
  const [planForm, setPlanForm] = useState({ free_mins_day: '', free_chats_day: '', charge_per_min: '' });
  ...
  return (
    ...
    <div style={{ maxWidth:'900px', margin:'0 auto', padding:'1.5rem 1rem' }}>
      ...
      {/* PLAN CONFIG TAB */}
      {tab === 'plan' && (
        <div style={{ background:'var(--color-background-primary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:'var(--border-radius-lg)', padding:'1.25rem', maxWidth:'400px' }}>
          ...
          <form onSubmit={savePlan} style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            ...
            <div>
              <label style={{ fontSize:'12px', color:'var(--color-text-secondary)', fontWeight:'500', display:'block', marginBottom:'4px' }}>रोज़ Free Chats (count)</label>
              <input type="number" value={planForm.free_chats_day} onChange={e => setPlanForm(f => ({...f, free_chats_day: e.target.value}))} />
            </div>
            <div>
              <label style={{ fontSize:'12px', color:'var(--color-text-secondary)', fontWeight:'500', display:'block', marginBottom:'4px' }}>Charge per Minute (₹) — limit के बाद</label>
          ...
          <p style={{ fontSize:'11px', color:'var(--color-text-tertiary)', marginTop:'1rem', marginBottom:0 }}>
            नोट: charge_per_min का payment integration अभी active नहीं है — सिर्फ config में सेव होगा future use के लिए।
          </p>
        </div>
      )}
    </div>
