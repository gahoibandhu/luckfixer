...
// Tier 1 miss (not a permanent outage), so Tier 1 now gets a second
...
export async function getPlanetPositions(dob, time, lat, lng, ayanamsa) {
  ...
  try {
    const result = await fetchPlanetPositionsPyswisseph(dob, time, lat, lng, ayanamsa, 5000);
    ...
    // to Tier 2 instead of wasting 800ms on a guaranteed repeat.
    if (process.env.EPHEMERIS_SERVICE_URL) {
      try {
        await sleep(800);
