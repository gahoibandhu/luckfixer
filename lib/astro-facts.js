...
import { evaluateSupportedWeakPlanet } from './neecha-bhanga.js';
import { pickGemstoneRecommendation } from './gemstone-policy.js';
import { evaluateSupportChain } from './graha-support-chain.js';
import { buildRemedyPlan } from './remedy-plan.js';
...
async function fetchPlanetPositionsPyswisseph(dob, time, lat, lng, ayanamsa, timeoutMs = 5000) {
  const url = process.env.EPHEMERIS_SERVICE_URL;
  if (!url) throw new Error('EPHEMERIS_SERVICE_URL not configured');

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), timeoutMs); // caller controls timeout — first attempt fast, retry attempt longer (covers Render cold start)
...
// ── Lightweight sidereal longitude lookup (Gochar Phal timeline) ────
// Purpose-built for scanning MANY dates quickly (e.g. finding exactly
// when Mars/Jupiter/Saturn change sign across a 5-year span) — unlike
// getPlanetPositions() above, this does NOT hit the pyswisseph network
// tier at all (would be far too slow/expensive for hundreds of sample
// points in one request) and does NOT compute retrograde/combustion/
// lagna — just the three slow-movers' sidereal sign for a given date,
// using the same local astronomy-engine + ayanamsa math as Tier 2
// above, so results stay consistent with the rest of the app.
const GOCHAR_PHAL_BODIES = { Mars: Astronomy.Body.Mars, Jupiter: Astronomy.Body.Jupiter, Saturn: Astronomy.Body.Saturn };

export function getSlowPlanetSigns(dateObj, ayanamsa) {
  const astroTime = Astronomy.MakeTime(dateObj);
  const ayVal = getAyanamsaDeg(dateObj, ayanamsa);
  const toSidereal = (tropDeg) => ((tropDeg - ayVal) % 360 + 360) % 360;

  const out = {};
  for (const name of Object.keys(GOCHAR_PHAL_BODIES)) {
    const lon = Astronomy.Ecliptic(Astronomy.GeoVector(GOCHAR_PHAL_BODIES[name], astroTime, true)).elon;
    const sidLon = toSidereal(lon);
    out[name] = { longitude: sidLon, sign: SIGNS[Math.floor(sidLon / 30)] };
  }
  return out;
}

// ── Moon-only lightweight lookup (Saptahik Phal / weekly forecast) ──
// Moon moves ~13°/day (full zodiac in ~27.3 days), so a week genuinely
// spans real, meaningfully different nakshatras — this is actual
// astronomical movement, not an invented "weekly" technique.
export function getMoonNakshatra(dateObj, ayanamsa) {
  const astroTime = Astronomy.MakeTime(dateObj);
  const ayVal = getAyanamsaDeg(dateObj, ayanamsa);
  const lon = Astronomy.Ecliptic(Astronomy.GeoVector(Astronomy.Body.Moon, astroTime, true)).elon;
  const sidLon = ((lon - ayVal) % 360 + 360) % 360;
  const nakIndex = Math.floor(sidLon / (360 / 27));
  return { longitude: sidLon, sign: SIGNS[Math.floor(sidLon / 30)], nakshatraIndex: nakIndex };
}

// ── Ephemeris unavailability — a distinct, catchable error type so
// callers (kundli route, admin migrations) can tell "no real
// astronomical data available right now" apart from ordinary bugs,
// and respond with "try again shortly" instead of silently saving a
// kundli built on fabricated planetary positions. See PREDICTION
// INTEGRITY note above getPlanetPositions() for why Tier 3 is no
// longer used as a silent automatic production fallback.
...
// ── PREDICTION INTEGRITY — Tier 3 (simulated/fabricated positions) is
// NOT used as a silent automatic fallback in production anymore. It
// previously was — meaning a real user could receive a full,
// confident-sounding Vedic reading built on sine-wave pseudo-data
// with zero indication anywhere (prompt, chat, or UI) that the
// underlying planetary positions weren't real. That's the exact
// fabrication problem this app's own anti-hallucination rules forbid
// at the AI layer — it just wasn't being enforced at the data layer.
//
// Render free-tier cold starts are the most common real cause of a
// Tier 1 miss (not a permanent outage), so Tier 1 gets a second
// attempt with a longer timeout before giving up — this alone should
// eliminate most Tier-3 fallbacks without any human intervention.
// Tier 2 (astronomy-engine) is REAL astronomical data, just slightly
// less precise for lunar nodes — kept as a legitimate fallback. Only
// when BOTH real tiers fail does this throw EphemerisUnavailableError
// for the caller to surface as "service busy, try again" — never as
// a saved, AI-narrated "reading".
//
// fastMode (used for the daily transit/gochar lookup on every chat
// message — NOT for natal chart creation): a cold Render instance can
// take 15-30s to spin up, so the default two-attempt retry (5s, then
// 12s more) can burn ~18s of a chat request's time budget before even
// reaching Tier 2 — squeezing whatever's left for the AI call itself
// and cascading into ITS fallback chain too. Day-level transit
// reporting (which house/sign a planet is in today) doesn't need
// arcsecond Swiss Ephemeris precision, so fastMode skips the slow
// retry entirely: one short attempt, then straight to Tier 2.
//
// calcPlanetPositionsSimulated() is kept in the file (still useful
// for local dev without EPHEMERIS_SERVICE_URL configured at all) but
// is only reachable when ALLOW_SIMULATED_EPHEMERIS=true is explicitly
// set — never by default, never silently in production.
export async function getPlanetPositions(dob, time, lat, lng, ayanamsa, fastMode = false) {
  ayanamsa = ayanamsa || 'lahiri';

  const attempts = [];

  try {
    const result = await fetchPlanetPositionsPyswisseph(dob, time, lat, lng, ayanamsa, fastMode ? 3000 : 5000);
    return { planets: result.planets, lagna: result.lagna, engine: 'pyswisseph' };
  } catch (e) {
    attempts.push({ tier: 'pyswisseph (attempt 1)', error: e.message });
    console.warn('[Ephemeris] Tier 1 attempt 1 failed:', e.message);

    // Only worth a slower retry for a TRANSIENT failure (timeout, 5xx,
    // cold start) — if the service isn't even configured, a second
    // attempt will fail identically and instantly, so skip straight
    // to Tier 2 instead of wasting 800ms on a guaranteed repeat. Also
    // skipped entirely in fastMode — see note above.
    if (process.env.EPHEMERIS_SERVICE_URL && !fastMode) {
      try {
        await sleep(800);
        const result = await fetchPlanetPositionsPyswisseph(dob, time, lat, lng, ayanamsa, 12000);
        return { planets: result.planets, lagna: result.lagna, engine: 'pyswisseph' };
      } catch (e2) {
        attempts.push({ tier: 'pyswisseph (attempt 2, extended timeout)', error: e2.message });
        console.warn('[Ephemeris] Tier 1 attempt 2 (extended timeout) failed:', e2.message);
      }
    }
  }
  ...
  try {
    ...
    attempts.push({ tier: 'astronomy-engine', error: e.message });
    console.error('[Ephemeris] Tier 2 (astronomy-engine) failed:', e.message);
  }

  if (process.env.ALLOW_SIMULATED_EPHEMERIS === 'true') {
    console.error('[Ephemeris] Both real tiers failed — ALLOW_SIMULATED_EPHEMERIS=true, using simulated data (dev/test only, should never be set in production).', attempts);
  ...
  console.error('[Ephemeris] Both real tiers failed — refusing to fall back to simulated data in production:', attempts);
  throw new EphemerisUnavailableError(
    'ज्योतिष गणना सेवा अभी उपलब्ध नहीं है — कृपया कुछ मिनट बाद पुनः प्रयास करें।',
    attempts
  );
}
...
export async function buildFactSheet(dob, time, lat, lng, ayanamsa) {
  ...
  // ── Neecha Bhanga (strict classical) + responsible gemstone gating —
  // computed once here, deterministically, so both the analysis prompt
  // AND live chat see the exact same values (no drift, no AI guessing).
  const neechaBhanga = lagna ? evaluateSupportedWeakPlanet(scored, lagna.sign) : [];
  const gemstoneGuidance = lagna ? pickGemstoneRecommendation(lagna.sign, scored) : { planet: null, reason: 'लग्न डेटा उपलब्ध नहीं।' };

  // ── Support-Chain compensation (see lib/graha-support-chain.js) —
  // for each weak graha, checks whether dispositor/exchange/aspect-
  // conjunct-friend/9th-lord support compensates before a remedy
  // targets the weak graha directly. remedyPlan then turns that
  // verdict + gemstoneGuidance + Lal Kitab data into one structured,
  // multi-system, combination-aware remedy object (see remedy-plan.js).
  const supportChain = lagna ? evaluateSupportChain(scored, lagna.sign, houseLords) : [];
  const remedyPlanInput = {
    weakestPlanet: { planet: weakest.name, name: weakest.nameHi },
    gemstoneGuidance,
    supportChain,
  };
  const remedyPlan = buildRemedyPlan(remedyPlanInput);
  ...
  return {
    ...
    strongestPlanet: { name: strongest.nameHi, planet: strongest.name, degree: strongest.degree, sign: strongest.signHi, dignity: strongest.dignityHi, score: strongest.strengthScore, house: strongest.house },
    weakestPlanet:   { name: weakest.nameHi, planet: weakest.name, degree: weakest.degree, sign: weakest.signHi, dignity: weakest.dignityHi, score: weakest.strengthScore, remedialWindow: weakest.remedialWindow, house: weakest.house },
