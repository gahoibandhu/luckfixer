...
// Each provider gets 6s (5 providers × 6s = 30s worst case) — paired with
// bumping vercel.json's maxDuration for this route to 45s, so there's a
// real buffer left over for the DB-save + usage-recording code that runs
// after the AI call returns, even in the worst case where every provider
// times out in sequence.
...
export async function getLuckfixerResponse(systemPrompt, userMessage, jsonMode = true) {
  ...
  for (const { name, fn } of providers) {
    try {
      const res = await withTimeout(fn, PROVIDER_TIMEOUT_MS, name);
