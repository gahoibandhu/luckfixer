...
// ── Per-provider timeout guard ─────────────────────────────────
// Without this, one slow/hung provider (e.g. Gemini silently hanging on
// a bad/deprecated model call) can consume the ENTIRE serverless
// function's maxDuration budget (30s for chat, see vercel.json) before
// it even throws — leaving zero time for the fallback chain to try the
// remaining 4 providers, so Vercel hard-kills the whole function and the
// browser gets a raw non-JSON error page instead of a graceful fallback
// reply. Each provider now gets its own short timeout so a bad one fails
// fast and the chain actually gets to move on, instead of failing silently.
// Per-provider, not one flat number — a flat 6000ms (the old value) was
// wrong for both ends of the chain: Gemini 3.7 is a "thinking" model
// (default medium reasoning) that routinely needs more than 6s for a
// plain chat reply, so it was timing out on EVERY request; OpenRouter's
// and HuggingFace's free-tier routing is also frequently slower than
// 6s. Gemini gets more time because it's the primary/highest-quality
// provider and (with thinkingLevel forced to 'low' below, see
// callGemini) usually finishes well under its budget anyway. Worst case
// now: 10+5+5+9+9 = 38s, paired with vercel.json's maxDuration bumped to
// 60s for this route, leaving a real buffer for the DB-save + usage-
// recording code that runs after the AI call returns.
...
function withTimeout(promiseFactory, ms, label) {
  ...
  return Promise.race([promiseFactory(controller.signal), timeout]);
}
...
async function callGemini(systemPrompt, userMessage, jsonMode = true, signal) {
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      ...
      // Gemini 3.x models default to 'medium' thinking, which is
      // needless latency for a short chat reply or a JSON kundli
      // analysis — this app never needs deep multi-step reasoning here,
      // just a fast, well-formatted answer. Forcing 'low' is what keeps
      // Gemini inside its PROVIDER_TIMEOUTS.Gemini budget in practice
      // instead of timing out on every single request. ('minimal' is
      // rejected by 3.7 Flash with a validation error, so 'low' is the
      // fastest legal setting — see https://ai.google.dev/gemini-api/docs/changelog)
...
// ── Groq model ID ───────────────────────────────────────────
// llama-3.3-70b-versatile was deprecated by Groq on June 17, 2026 (see
// https://console.groq.com/docs/deprecations) and now 404s on every
// call — which silently knocked Groq, the #2 link in the fallback
// chain, out of rotation entirely. openai/gpt-oss-120b is Groq's own
// recommended replacement for it.
const GROQ_MODEL = process.env.GROQ_MODEL || 'openai/gpt-oss-120b';

// ── 2. SambaNova (OpenAI-compatible, very fast) ───────────────
async function callSambaNova(systemPrompt, userMessage, jsonMode = true, signal) {
  const keys = [
...
// ── 3. OpenRouter (multiple free models) ─────────────────────
async function callOpenRouter(systemPrompt, userMessage, jsonMode = true, signal) {
...
async function callHuggingFace(systemPrompt, userMessage, jsonMode = true, signal) {
...
// ── 5. Groq ──────────────────────────────────────────────────
async function callGroq(systemPrompt, userMessage, jsonMode = true, signal) {
  ...
  if (jsonMode) {
    text = text.replace(/^```json\s*/i, '').replace(/^```\s*/i, '').replace(/\s*```$/i, '').trim();
    return { content: JSON.parse(text), model: `groq/${GROQ_MODEL}` };
  }
  return { content: text, model: `groq/${GROQ_MODEL}` };
}
...
export async function getLuckfixerResponse(systemPrompt, userMessage, jsonMode = true) {
  ...
  const providers = [
    { name: 'Gemini', fn: (signal) => callGemini(systemPrompt, userMessage, jsonMode, signal) },
    { name: 'Groq', fn: (signal) => callGroq(systemPrompt, userMessage, jsonMode, signal) },
    { name: 'SambaNova', fn: (signal) => callSambaNova(systemPrompt, userMessage, jsonMode, signal) },
    { name: 'OpenRouter', fn: (signal) => callOpenRouter(systemPrompt, userMessage, jsonMode, signal) },
    { name: 'HuggingFace', fn: (signal) => callHuggingFace(systemPrompt, userMessage, jsonMode, signal) },
  ];

  for (const { name, fn } of providers) {
    try {
      const res = await withTimeout(fn, PROVIDER_TIMEOUTS[name], name);
  ...
  return {
    content: {
      ...
      analytical_insight: 'अभी काफी ज्यादा users active हैं, इसलिए हमारे सभी AI ज्योतिषी बॉट व्यस्त हैं। कृपया कुछ देर बाद पुनः प्रयास करें।',
...
export async function getChatResponse(systemPrompt, messages, langPref = 'auto') {
  ...
  const chatProviders = [
    ...
    { name: 'Groq', fn: (signal) => callGroq(systemPrompt, prompt, false, signal) },
    { name: 'SambaNova', fn: (signal) => callSambaNova(systemPrompt, prompt, false, signal) },
    { name: 'OpenRouter', fn: (signal) => callOpenRouter(systemPrompt, prompt, false, signal) },
    { name: 'HuggingFace', fn: (signal) => callHuggingFace(systemPrompt, prompt, false, signal) },
  ];
  ...
  return {
    content: lang === 'en'
      ? "We're seeing heavy traffic right now — all our bots are busy. Please try again in a little while."
      : 'अभी बहुत ज्यादा users active हैं, इसलिए हमारे सभी बॉट व्यस्त हैं। कृपया कुछ देर बाद फिर से कोशिश करें।',
    model: 'fallback',
    fallback_used: true,
  };
}
