...
export function buildSaptahikPhal(ayanamsa = 'lahiri', weakestPlanetKey = null) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    days.push({
      ...
      combinedNote: `चंद्रमा ${nakshatra} नक्षत्र में है — ${nakshatraNote}। आज का दिन-स्वामी ${dayLord} है, इसलिए ${dayNote} के लिए यह समय अनुकूल रहेगा।`,
    });
  }
...
export function formatSaptahikForPrompt(saptahik) {
  ...
  saptahik.days.forEach(d => {
    out += `\n${d.dayName} (${formatDateDDMMYYYY(d.date)}): चंद्र ${d.nakshatra} में — ${d.nakshatraNote}; दिन-स्वामी ${d.dayLord} — ${d.dayNote}`;
  });
  if (saptahik.remedy) {
