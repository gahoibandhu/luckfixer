...
// ── Manglik (Mangal Dosh) compatibility check ──────────────────
// The single most commonly asked-about factor in real-world Indian
// marriage matching — often discussed even more than the 36-guna
// score itself — yet the Ashtakoot system above doesn't cover it at
// all (it's a separate classical check). Classical rule: Mars in
// house 1, 2, 4, 7, 8, or 12 from Lagna. Traditional matching
// guidance: if only ONE partner is Manglik, it's flagged as needing
// attention/remedy; if BOTH are Manglik, it's traditionally considered
// to cancel out (Manglik-Manglik pairing is standard practice); if
// NEITHER is Manglik, no concern.
const MANGLIK_HOUSES = [1, 2, 4, 7, 8, 12];

function checkManglik(factSheet) {
  const lagnaSign = factSheet?.lagna?.sign;
  const mars = factSheet?.planets?.find(p => p.name === 'Mars');
  if (!lagnaSign || !mars) return { isManglik: false, house: null };

  const li = SIGNS.indexOf(lagnaSign);
  const mi = SIGNS.indexOf(mars.sign);
  if (li === -1 || mi === -1) return { isManglik: false, house: null };

  const house = ((mi - li + 12) % 12) + 1;
  const isManglik = MANGLIK_HOUSES.includes(house);
  // Classical cancellation: Mars in own sign or exalted meaningfully
  // softens the dosha's severity, same as in the individual yoga check.
  const isCancelled = mars.dignityHi === 'स्वराशि' || mars.dignityHi === 'उच्च';
  return { isManglik, house, isCancelled, sign: mars.signHi };
}

function buildManglikCompatibility(boyManglik, girlManglik) {
  if (!boyManglik.isManglik && !girlManglik.isManglik) {
    return {
      status: 'clear',
      statusHi: 'कोई मंगल दोष नहीं',
      note: 'दोनों की कुंडली में मंगल दोष नहीं है — इस दृष्टि से कोई बाधा नहीं।',
    };
  }
  if (boyManglik.isManglik && girlManglik.isManglik) {
    return {
      status: 'both',
      statusHi: 'दोनों मांगलिक — स्वतः संतुलित',
      note: `दोनों की कुंडली में मंगल दोष है (वर: ${boyManglik.house}वें भाव, वधू: ${girlManglik.house}वें भाव) — पारंपरिक मान्यता अनुसार दोनों का मंगल दोष एक-दूसरे को संतुलित कर देता है, इसे शुभ माना जाता है।`,
    };
  }
  const whoIs = boyManglik.isManglik ? 'वर' : 'वधू';
  const which = boyManglik.isManglik ? boyManglik : girlManglik;
  return {
    status: 'one_sided',
    statusHi: `केवल ${whoIs} मांगलिक — ध्यान दें`,
    note: `${whoIs} की कुंडली में मंगल दोष है (${which.house}वें भाव में)${which.isCancelled ? ', लेकिन मंगल की स्थिति के कारण इसका प्रभाव हल्का है' : ''} जबकि दूसरे पक्ष में नहीं — पारंपरिक रूप से इसे ध्यान देने योग्य माना जाता है। इसका मतलब विवाह न हो, यह नहीं — कई परिवार पूजा-पाठ या उपाय के साथ आगे बढ़ते हैं। किसी अनुभवी पंडित से मंगल दोष शांति पूजा के बारे में परामर्श लेना पारंपरिक तरीका है।`,
  };
}
...
export function calcKundliMilan(boyFactSheet, girlFactSheet) {
  ...
  const boyManglik  = checkManglik(boyFactSheet);
  const girlManglik = checkManglik(girlFactSheet);
  const manglikCompat = buildManglikCompatibility(boyManglik, girlManglik);
  if (manglikCompat.status === 'one_sided') {
    doshas.push({ name: 'मंगल दोष (एकतरफा)', severity: 'medium', effect: manglikCompat.note });
  }
  ...
  return {
    ...
    verdictHi,
    doshas,
    manglik: { boy: boyManglik, girl: girlManglik, compatibility: manglikCompat },
