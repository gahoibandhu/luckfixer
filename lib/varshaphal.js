...
export function buildVarshaphal(factSheet, dob) {
  ...
  // Which "varsh" (solar year, birthday-to-birthday) are we currently in?
  // Compare using this calendar year's birthday date, month/day only.
  const thisYearBirthday = new Date(currentYear, birthDate.getMonth(), birthDate.getDate());
  const isBirthdayPast = today >= thisYearBirthday;
  const varshYear = isBirthdayPast ? currentYear : currentYear - 1;
  const varshEndYear = varshYear + 1;

  // BUG FIX: solarReturnDate must be anchored to varshYear (the year the
  // CURRENT solar year actually started), not currentYear. Previously this
  // used `currentYear` unconditionally — so whenever today's date fell
  // before this year's birthday (e.g. birthday in October, viewing in
  // August), varshYear correctly rolled back to last year but
  // solarReturnDate stayed in the future (this year's not-yet-occurred
  // birthday). muddaDasha is built by walking forward exactly one solar
  // year from solarReturnDate — so its periods were silently shifted a
  // full year into the future and never overlapped "today", making the
  // मासिक (monthly) tab permanently empty until that future birthday
  // actually arrived. This was invisible in the yearly/weekly tabs
  // because those don't filter by "does this period contain today".
  const solarReturnDate = new Date(varshYear, birthDate.getMonth(), birthDate.getDate());
