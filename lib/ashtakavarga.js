...
export function buildAshtakavarga(planets, lagnaSign) {
  ...
  // CRITICAL: Shodhana must be applied to each planet's individual bindu
  // array BEFORE summing into Sarvashtakavarga. Applying it to the combined
  // sarva array is mathematically wrong — it loses per-planet zero-bindu
  // information that makes trine reduction meaningful.
  // Source: BPHS ch.66, Phala Deepika ch.19.
  const shodhana = (arr) => {
    const result = [...arr];
  ...
  // Apply shodhana per planet array, THEN sum into reduced sarva
  const byPlanetReduced = {};
  const sarvaAfterShodhana = new Array(12).fill(0);
  for (const [planet, bindus] of Object.entries(byPlanet)) {
    const reduced = shodhana(bindus);
    byPlanetReduced[planet] = reduced;
    for (let i = 0; i < 12; i++) sarvaAfterShodhana[i] += reduced[i];
  }
