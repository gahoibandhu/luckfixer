...
// ── Yoga-activation period scanner ──────────────────────────────
// This is the real classical technique the person's message is asking
// for: for a given life-area (marriage, children, career...), scan the
// person's ENTIRE dasha timeline (birth to ~120 years — already fully
// computed above in `mahadashas`, not just the current period) and find
// every Mahadasha/Antardasha combination ruled by that area's relevant
// planets (house lord + classical karaka). These are the genuine
// "active windows" for that life event — both past (useful to explain
// why something already happened when it did) and future (genuine
// predicted timing) — rather than improvising a vague "next few months"
// window from only the CURRENT dasha.
//
// A period where BOTH the Mahadasha lord AND Antardasha lord are
// relevant is classically the strongest window (double connection to
// the karaka/house); a period where only one of the two matches is a
// moderate window.
export function findYogaPeriods(mahadashas, relevantLords) {
  const periods = [];
  for (const md of mahadashas) {
    const mdRelevant = relevantLords.includes(md.lord);
    for (const ad of md.antarDashas) {
      const adRelevant = relevantLords.includes(ad.lord);
      if (mdRelevant || adRelevant) {
        periods.push({
          mdLord: md.lord, mdLordHi: md.lordHi,
          adLord: ad.lord, adLordHi: ad.lordHi,
          start: ad.start,
          end: ad.end,
          strength: (mdRelevant && adRelevant) ? 'strong' : 'moderate',
        });
      }
    }
  }
