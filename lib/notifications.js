...
// ── Daily remedy reminder email — opt-in only (see migration_016) ──
// One combined email per user listing today's day-matched pending
// Lal Kitab remedies (mantra/gemstone/dosha remedies have no fixed
// day, so they aren't included here — they're always visible in-app).
export async function sendRemedyReminderEmail(toEmail, userName, todaysRemedies, weekday) {
  if (!resend) {
    console.warn('[Notifications] RESEND_API_KEY not set — skipping email');
    return { skipped: true };
  }
  ...
  const name = userName?.split(' ')[0] || 'दोस्त';
  const itemsHtml = todaysRemedies.map(r => `
    <li style="margin-bottom:8px;color:#333;font-size:14px;line-height:1.6;">
      <strong>${r.planet_hi || r.planet || ''}</strong> — ${r.donate || ''}${r.avoid ? ` (सावधानी: ${r.avoid})` : ''}
    </li>`).join('');

  const body = `
    <p style="font-size:16px;color:#1a1a18;margin:0 0 12px;">नमस्ते ${name} जी! 🙏</p>
    <p style="font-size:14px;color:#333;line-height:1.6;margin:0 0 16px;">
      आज <strong>${weekday}</strong> है — आपके लिए ये उपाय आज के दिन के लिए हैं:
    </p>
    <ul style="padding-left:18px;margin:0 0 16px;">${itemsHtml}</ul>
  `;
  return await resend.emails.send({
    from: FROM_EMAIL,
    to: toEmail,
    subject: `${name} जी, आज ${weekday} है — आपका उपाय याद दिला रहे हैं 🪔`,
    html: wrapEmailHtml(body),
  });
}
...
export async function sendBroadcastEmail({ recipients, subject, headline, bodyText, ctaLabel, ctaUrl }) {
  ...
  // ── Rate limit: exactly 10 requests/second, no more ──────────
  // Sends 10 emails, waits for the full second to elapse, then sends
  // the next 10 — regardless of how fast Resend responds. This keeps
  // us safely under Resend's per-second cap and avoids 429 errors on
  // larger broadcasts.
  const RATE_PER_SECOND = 10;
  for (let i = 0; i < recipients.length; i += RATE_PER_SECOND) {
    const batch = recipients.slice(i, i + RATE_PER_SECOND);
    ...
    // Wait out the remainder of the second before the next batch of 10,
    // so we never exceed 10 requests/second even if Resend responds fast.
    if (i + RATE_PER_SECOND < recipients.length) {
