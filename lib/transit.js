...
export async function buildTransitReport(natalFactSheet, lat, lng) {
  ...
  // fastMode: this runs on every chat message (not kundli creation),
  // so it can't afford the ~18s worst-case wait a cold Render instance
  // would otherwise cost here — see the note on getPlanetPositions.
  const result = await getPlanetPositions(todayStr, nowTime, lat, lng, 'lahiri', true);
