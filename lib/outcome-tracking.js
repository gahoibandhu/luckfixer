...
// ── Site-wide dasha-context accuracy pattern (see migration_014) ──
// Deliberately gated: a small sample would give a falsely precise-
// looking percentage (the exact same reasoning as MIN_TRACKED_FOR_DISPLAY
// in app/api/chat/route.js for personal accuracy) — here applied to a
// SITE-WIDE aggregate instead of one user's history.
const MIN_SAMPLE_FOR_DASHA_STAT = 8;

export async function getDashaAccuracyStat(supabase, predictionType, dashaContext) {
  if (!predictionType || !dashaContext) return null;
  const { data } = await supabase
    .from('dasha_accuracy_stats')
    .select('responded, positive')
    .eq('prediction_type', predictionType)
    .eq('dasha_context', dashaContext)
    .maybeSingle();

  if (!data || (data.responded || 0) < MIN_SAMPLE_FOR_DASHA_STAT) return null;

  return {
    responded:   data.responded,
    positive:    data.positive,
    positivePct: Math.round((data.positive / data.responded) * 100),
  };
}

// ── Remedy-completion vs outcome correlation (see migration_015) ──
// Same honesty gate — BOTH groups (took remedy / didn't) need a
// minimum sample before this is used, otherwise a lucky/unlucky small
// group would produce a misleading comparison.
const MIN_SAMPLE_FOR_REMEDY_CORRELATION = 10;

export async function getRemedyOutcomeCorrelation(supabase) {
  const { data } = await supabase
    .from('remedy_outcome_correlation')
    .select('took_remedy, responded, positive');

  if (!data || data.length < 2) return null;

  const took    = data.find(r => r.took_remedy === true);
  const didNot  = data.find(r => r.took_remedy === false);

  if (!took || !didNot) return null;
  if (took.responded < MIN_SAMPLE_FOR_REMEDY_CORRELATION || didNot.responded < MIN_SAMPLE_FOR_REMEDY_CORRELATION) return null;

  return {
    tookRemedy:   { responded: took.responded,   positivePct: Math.round((took.positive / took.responded) * 100) },
    noRemedy:     { responded: didNot.responded, positivePct: Math.round((didNot.positive / didNot.responded) * 100) },
  };
}
