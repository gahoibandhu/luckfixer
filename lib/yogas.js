...
export function detectYogas(planets, lagnaSign, houseLords, d9Chart) {
  ...
  if (jupiterHouse && rahuHouse && jupiterHouse === rahuHouse) {
    yogas.push({
      ...
      remedyPlanets: ['Jupiter', 'Rahu'],
    });
  }


  // ── नीच भंग राजयोग — STRICT classical criteria only ─────────
  // (see lib/neecha-bhanga.js — deliberately NOT "any benefic aspect",
  // only the four named classical Parashari conditions count)
  const CRITERIA_LABEL_HI = {
    dispositorInKendra: 'राशि स्वामी लग्न/चंद्र से केंद्र में स्थित है',
    exaltationLordInKendra: 'उस राशि का उच्च-स्वामी लग्न/चंद्र से केंद्र में स्थित है',
    dispositorKendraFromExaltLord: 'राशि स्वामी और उच्च-स्वामी परस्पर केंद्र में हैं',
    dispositorAspectsPlanet: 'राशि स्वामी स्वयं इस ग्रह को दृष्टि दे रहा है',
  };
  const neechaBhangaResults = evaluateSupportedWeakPlanet(planets, lagnaSign);
  for (const nb of neechaBhangaResults) {
    if (!nb.isNeechaBhanga) continue;
    const metLabels = Object.entries(nb.criteriaMet).filter(([, v]) => v).map(([k]) => CRITERIA_LABEL_HI[k]);
    yogas.push({
      name: 'नीच भंग राजयोग',
      category: 'neecha_bhanga',
      strength: 'medium',
      lifeArea: `${nb.planetHi} से जुड़ा क्षेत्र`,
      description: `${nb.planetHi} ${nb.signHi} राशि में नीच का है, लेकिन इसकी नीचता भंग (cancel) हो रही है क्योंकि: ${metLabels.join('; ')}। शास्त्रीय रूप से इसका अर्थ है — शुरुआत में इस ग्रह से जुड़े क्षेत्र में संघर्ष या देरी दिख सकती है, लेकिन समय के साथ यह ग्रह वास्तव में सकारात्मक फल देने में सक्षम हो जाता है (सिर्फ "निष्क्रिय" नहीं मानना चाहिए)।`,
      source: 'Parashari classical criteria (BPHS)',
      planet: nb.planet,
      validationQ: `क्या ${nb.planetHi} से जुड़े क्षेत्र में शुरुआत में मुश्किल रही, फिर बेहतर होती गई?`,
    });
  }
...
export function formatYogasForPrompt(yogas) {
  ...
  if (challenging.length) out += `\nचुनौतीपूर्ण योग:\n${challenging.map(y => `• ${y.description}${y.remedy ? `\n  उपाय: ${y.remedy}` : ''}`).join('\n')}`;
  return out;
}
