...
// ── Per-planet segment scan: finds every sign-change boundary for ONE
// planet within [scanStart, scanEnd] and returns its {sign, house,
// start, end, text} segments. Shared by buildGocharPhalTimeline (wide,
// multi-year window, per-planet lists) and buildAnnualTransitPeriods
// (birthday-bounded window, merged across all three planets).
function computePlanetSegments(planet, moonSign, ayanamsa, scanStart, scanEnd) {
  ...
  // Close out the final open segment at scan end
  if (prevSign !== null) {
    const house = houseFromMoon(prevSign, moonSign);
    segments.push({
      planet,
...
export function buildGocharPhalTimeline(moonSign, ayanamsa = 'lahiri', yearsBack = 2, yearsForward = 3) {
  ...
  const merged = [
    ...computePlanetSegments('Mars', moonSign, ayanamsa, scanStart, scanEnd),
    ...computePlanetSegments('Jupiter', moonSign, ayanamsa, scanStart, scanEnd),
    ...computePlanetSegments('Saturn', moonSign, ayanamsa, scanStart, scanEnd),
  ];
  merged.sort((a, b) => new Date(a.start) - new Date(b.start));
  return merged;
}

// ── Annual (birthday-to-birthday) transit periods — MERGED across all
// three planets, not per-planet. A new period starts whenever ANY of
// Mars/Jupiter/Saturn changes sign or house-from-Moon; within a period
// nothing changes, so it can be narrated as one coherent phase. This is
// the "varsh" window (solarReturnDate to solarReturnDate + 1 year),
// matching how Varshaphal itself defines the year — not the calendar
// Jan-Dec year.
export function buildAnnualTransitPeriods(moonSign, ayanamsa = 'lahiri', solarReturnDate) {
  if (!moonSign || !solarReturnDate) return [];

  const scanStart = new Date(solarReturnDate);
  const scanEnd = new Date(solarReturnDate);
  scanEnd.setFullYear(scanEnd.getFullYear() + 1);

  const byPlanet = {
    Mars: computePlanetSegments('Mars', moonSign, ayanamsa, scanStart, scanEnd),
    Jupiter: computePlanetSegments('Jupiter', moonSign, ayanamsa, scanStart, scanEnd),
    Saturn: computePlanetSegments('Saturn', moonSign, ayanamsa, scanStart, scanEnd),
  };

  // Union of every segment boundary across all three planets, sorted —
  // these are exactly the dates where "kuch change ho raha hai".
  const boundarySet = new Set([
    scanStart.toISOString().slice(0, 10),
    scanEnd.toISOString().slice(0, 10),
  ]);
  for (const planet of ['Mars', 'Jupiter', 'Saturn']) {
    for (const seg of byPlanet[planet]) {
  ...
  for (let i = 0; i < boundaries.length - 1; i++) {
    ...
    const planetsActive = ['Mars', 'Jupiter', 'Saturn'].map(planet => {
      const seg = byPlanet[planet].find(s => s.start <= start && s.end >= end);
      return seg ? { planet, planetHi: PLANETS_HI[planet], sign: seg.sign, house: seg.house, text: seg.text } : null;
    }).filter(Boolean);
    periods.push({ start, end, planets: planetsActive });
  }
  return periods;
}

export function formatAnnualTransitPeriodsForPrompt(periods) {
  if (!periods || periods.length === 0) return '';
  let out = '\nANNUAL TRANSIT PERIODS (birthday-to-birthday year, real calculated boundaries — write one narrative per period, in this exact order, using these exact dates):';
  periods.forEach((p, i) => {
    const planetsStr = p.planets.map(pl => `${pl.planetHi} ${pl.house}वें भाव में`).join(', ');
    out += `\n${i + 1}. ${p.start} से ${p.end}: ${planetsStr}`;
  });
  return out;
}
