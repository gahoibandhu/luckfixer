...
// Classical Parashari/BPHS planetary combinations (conjunctions, house
// placements, sign exchanges) and Lal Kitab remedies, deterministically
// matched against the fact-sheet and injected into the AI prompt — AI
// does narrative only, logic is here.
//
// NOTE: This is Parashari + Lal Kitab only. It does NOT implement genuine
// Bhrigu Nandi Nadi techniques (which use nakshatra-pada-level micro-rules
// from oral tradition with low standardization across schools) — any
// earlier "(Nadi)" labels in this file were inaccurate and have been
// corrected. If true Nadi-style analysis is added in future, it should
// be a clearly separate, explicitly scoped module.
...
const YOGA_RULES = [
  ...
  // Moon combinations
  { rule: 'Moon_Rahu_conj', match: p => hasConjunction(p, 'Moon', 'Rahu', 8), text: 'चंद्र-राहु युति: मानसिक अशांति, असाधारण विचार, विदेशी संपर्क, अपरंपरागत जीवन शैली (शास्त्रीय सिद्धांत)', past_question: 'क्या आप कभी-कभी बहुत अकेलापन या मानसिक अशांति महसूस करते हैं?' },
  { rule: 'Moon_Ketu_conj', match: p => hasConjunction(p, 'Moon', 'Ketu', 8), text: 'चंद्र-केतु युति: आध्यात्मिक झुकाव, भावनात्मक वैराग्य, माता से दूरी (शास्त्रीय सिद्धांत)', past_question: 'क्या आपकी माता या परिवार से कोई भावनात्मक दूरी रही है?' },
  ...
  // Rahu/Ketu
  { rule: 'Rahu_10th', match: (p, h) => inHouse(p, 'Rahu', 10, h), text: 'राहु दशम: technology, media या अपरंपरागत क्षेत्र में करियर, विदेश से जुड़ा काम (BPHS)', past_question: 'क्या आपका काम technology, media या किसी नए/अलग क्षेत्र से जुड़ा है?' },
  { rule: 'Ketu_1st', match: (p, h) => inHouse(p, 'Ketu', 1, h), text: 'केतु लग्न: आध्यात्मिक स्वभाव, विरक्त व्यक्तित्व, पूर्वजन्म की विशेष शक्तियां (BPHS)', past_question: 'क्या आप स्वभाव से अंतर्मुखी या आध्यात्मिक हैं?' },
...
// ── One concise, ready-to-display remedy line built from the table
// above — shared across features (Masik/Saptahik faladesh, chat
// remedy rule) so remedies stay consistent wherever they're shown.
function buildRemedyLine(planet) {
  const r = LAL_KITAB_REMEDIES[planet];
  if (!r) return null;
  return `${r.day} को ${r.donate} का दान करें और "${r.mantra}" मंत्र का ${r.count} बार जाप करें।`;
}
...
export { LAL_KITAB_REMEDIES, DASHA_PAST_VALIDATION, buildRemedyLine };
